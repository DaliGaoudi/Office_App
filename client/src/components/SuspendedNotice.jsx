 ...
 * "أعد المحاولة" picks up a reinstatement within seconds of it being made.
 *
...
export default function SuspendedNotice({ status, message, providerContact, canExport, onRetry, onLogout }) {
  ...
  const exportData = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('تعذّر تصدير البيانات.');
      ...
      setError(e.message || 'تعذّر تصدير البيانات.');
    } finally {
      setBusy(false);
    }
  };
    ...
    <div style={{
      ...
      <div className="glass" style={{ maxWidth: '620px', width: '100%', padding: '2.5rem', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: '1.5rem', marginBottom: '0.75rem' }}>
          {terminated ? 'انتهى العقد وتم إيقاف الخدمة' : 'الخدمة موقوفة مؤقتاً'}
        </h1>

        <p style={{ color: 'var(--text-muted)', lineHeight: 1.9, marginBottom: '1.5rem' }}>
          {message || 'تم إيقاف الخدمة. يُرجى التواصل مع المزوّد لتسوية الوضعية.'}
        </p>

        {providerContact && (
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
            للتواصل: <span dir="ltr">{providerContact}</span>
          </p>
        )}
        ...
        <p style={{
          ...
          بياناتكم محفوظة بالكامل ولم يُمسّ منها شيء. بمجرّد رفع الإيقاف يعود العمل كما كان.
          {canExport && ' ويمكنكم في كل الأحوال تنزيل نسخة كاملة من سجلاتكم.'}
        </p>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
          {canExport && (
            <button className="btn" onClick={exportData} disabled={busy}>
              <Download size={18} /> تنزيل نسخة من البيانات
            </button>
          )}
          <button
            className="btn"
