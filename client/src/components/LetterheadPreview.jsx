  ...
  return (
    <div dir="ltr" style={{
      ...
      <div style={{ textAlign: 'center', borderBottom: '2px solid #222', paddingBottom: '0.8rem', marginBottom: '1rem' }}>
        <div style={{ fontSize: '0.95rem' }}>Maître</div>
        <div style={{ fontSize: '1.35rem', fontWeight: 700, margin: '0.15rem 0' }}>{val(profile.officeName, 'Bailiff name')}</div>
        <div style={{ fontSize: '0.95rem' }}>Judicial Enforcement Officer, {ph(profile.officeCity, '…')}</div>
        <div style={{ fontSize: '0.85rem', marginTop: '0.35rem' }}>{val(profile.officeAddress, 'Address')}</div>
        <div style={{ fontSize: '0.85rem' }}>
          Phone: {val(profile.officePhone, '—')} &nbsp;–&nbsp; Fax: {val(profile.officeFax, '—')}
        </div>
        <div style={{ fontSize: '0.85rem' }}>Tax ID: {val(profile.taxId, '—')}</div>
      </div>

      {/* Act opening — mirrors the body of template_cnss.docx */}
      <h3 style={{ textAlign: 'center', margin: '0.5rem 0 1rem' }}>NOTIFICATION ACT — LIQUIDATION CARD</h3>
      <p style={{ margin: '0 0 0.8rem' }}>
        At the request of the National Social Security Fund, through its regional office{' '}
        <b>{ph(profile.cnssBureau, '………')}</b>.
      </p>
      <p style={{ margin: '0 0 0.8rem', textAlign: 'center', fontWeight: 700 }}>
        We, Maître {ph(profile.officeName, '………')}, Judicial Enforcement Officer<br />
        of the judicial district of {ph(profile.officeJurisdiction, '………')}<br />
        of {ph(profile.officeAddress, '………')}, undersigned
      </p>
      <p style={{ margin: '0 0 0.8rem' }}>
        … rendered enforceable, on behalf of the Governor and by his delegation, by the Regional Director of Social Affairs of{' '}
        <b>{ph(profile.cnssRegion, '………')}</b>, as follows:
      </p>

      {/* Sample card table */}
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem', textAlign: 'center' }}>
        <thead>
          <tr>{['Card no.', 'Quarter', 'Principal debt', 'Penalty', 'Penalty start date'].map((h) => (
      ...
      <div style={{ textAlign: 'center', marginTop: '1rem', fontSize: '0.85rem' }}>THE JUDICIAL ENFORCEMENT OFFICER</div>
    </div>
  );
}
