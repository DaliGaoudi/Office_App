 ...
 * first-run onboarding screen and editable afterwards in Settings. It is fetched at
...
export const GENERIC_OFFICE_NAME = 'Bailiff Office Suite';
...
/*
 * The bailiff's name is stored bare ("Mourad Gaoudi"); the sidebar shows it as
 * "Office of Maître …". Compose it here so the stored value stays the same string
 * the act template merges.
 */
export const composeOfficeLabel = (officeName) => {
    const name = (officeName || '').trim();
    return name ? `Office of Maître ${name}` : GENERIC_OFFICE_NAME;
};
